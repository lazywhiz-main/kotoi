# KOTOI — Cursor 実装ハンドオフパッケージ

> KOTOI は「雑多に放り込むと、AIが分類し・要約し・調べ、そして"次の問い"を返す」個人用ノートアプリ。答えではなく**問いを増やす**ことに全処理を収束させるのが独自性。

このフォルダは、Cursor（AIコーディング）に渡してKOTOIを実装するための仕様＋雛形一式です。**フル実装コードは含みません**。Cursorがこの仕様に沿ってコードを生成します。
...
見取り図の**保存**（端末）と**投稿**（外の SNS）は別体験。「アプリ内シェア」とは言わない（KOTOI 内の他人への受け渡しに聞こえるため）。
