...
問いの質が陳腐だとKOTOIは死ぬ。ここは反復改善する前提で、プロンプトを独立管理・バージョン管理すること。すべて Edge Function から Anthropic Messages API を呼ぶ。モデルは既定 `claude-sonnet-5`（品質重視の処理は上位モデルに切替可）。**出力はJSONで受け、Zod等で検証**してから保存する。
